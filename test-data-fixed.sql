-- TeleMedCare V11.0 Test Data (Compatible with existing schema)
-- Dati di esempio per testing del sistema

-- First check existing leads and get the next ID
-- Insert test leads with proper structure
INSERT INTO leads (
...
-- Get the IDs of the inserted leads (assuming they will be 1, 2, 3, 4)
-- Now insert test assistiti for the first 3 leads (simulating conversion)
INSERT OR IGNORE INTO assistiti (
  ...
  (2, 'ASS202410060002', 'Anna', 'Verdi', 'giulia.verdi@email.com', '3339876543', '1965-07-22',
...
INSERT OR IGNORE INTO system_logs (tipo, modulo, messaggio, dettagli, livello, assistito_id, lead_id, timestamp) VALUES
  ...
  ('LEAD_CREATED', 'LeadCore', 'Nuovo lead registrato da form web', 
   '{"source": "web_form", "pacchetto": "Base"}', 'INFO', NULL, 4, '2024-10-06 08:00:00'),
  
  ('EMAIL_SENT', 'AutomationService', 'Email NOTIFICA_INFO inviata a laura.gialli@email.com', 
   '{"templateId": "NOTIFICA_INFO", "recipient": "laura.gialli@email.com"}', 'INFO', NULL, 4, '2024-10-06 08:05:00');

-- Update lead status to CONVERTED for first 3 leads
UPDATE leads SET status = 'CONVERTED' WHERE id IN (1, 2, 3);