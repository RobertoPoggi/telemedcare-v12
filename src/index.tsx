...
app.get('/api/contratti/:id/pdf-print', async (c) => {
  ...
  try {
    ...
    // Recupera contratto con dati lead E contenuto_html originale
    const contract = await c.env.DB.prepare(`
      SELECT c.*, 
        l.nomeRichiedente, l.cognomeRichiedente, l.email, l.telefono,
        l.cfIntestatario, l.indirizzoIntestatario,
    ...
    // ✅ Se esiste contenuto_html originale, usalo come base e aggiungi la firma
    if (contract.contenuto_html && contract.status === 'SIGNED' && contract.signature_data) {
      // Inserisci la firma grafica nell'HTML originale
      let htmlWithSignature = contract.contenuto_html
      
      // Cerca il placeholder della firma o aggiungi alla fine
      const signatureHtml = `
        <div style="margin-top: 40px; padding: 20px; border: 2px solid #16a34a; background: #f0f9ff; border-radius: 8px; page-break-inside: avoid;">
          ...
          <div style="padding: 15px; background: white; border: 1px solid #cbd5e1; text-align: center;">
            <img src="${contract.signature_data}" alt="Firma Digitale" style="max-width: 400px; max-height: 120px; border: 1px solid #e2e8f0; padding: 10px;" />
            <div style="margin-top: 10px; font-size: 12px; color: #64748b;">
              ...
              <p style="margin: 5px 0;"><strong>IP:</strong> ${contract.signature_ip || 'N/A'}</p>
              <p style="margin: 5px 0;"><strong>Timestamp:</strong> ${contract.signature_timestamp ? new Date(contract.signature_timestamp).toLocaleString('it-IT') : 'N/A'}</p>
            </div>
          </div>
        </div>
      `
      
      // Inserisci prima del tag </body>
      if (htmlWithSignature.includes('</body>')) {
        htmlWithSignature = htmlWithSignature.replace('</body>', `${signatureHtml}</body>`)
      } else {
        htmlWithSignature += signatureHtml
      }
      
      // Aggiungi stili per stampa e pulsante
      const printStyles = `
        <style>
          @media print {
      ...
      if (htmlWithSignature.includes('</head>')) {
        htmlWithSignature = htmlWithSignature.replace('</head>', `${printStyles}</head>`)
      } else {
        htmlWithSignature = `<head>${printStyles}</head>` + htmlWithSignature
      }
      
      return c.html(htmlWithSignature)
    }
...
// GET /test-insert-manual-contracts - Pagina inserimento contratti manuali
app.get('/test-insert-manual-contracts', async (c) => {
  return c.html(`
<!DOCTYPE html>
<html lang="it">
<head>
...
<body>
  <div class="container">
    <h1>📝 Inserimento Contratti Manuali</h1>
    
    <div class="warning">
      <strong>⚠️ Attenzione:</strong> Questo strumento inserisce contratti per <strong>lead già esistenti</strong> nel database.<br>
      Se un lead non viene trovato o ha già un contratto, l'operazione verrà saltata con un messaggio di errore.
    </div>
    ...
    <button id="insertBtn" onclick="insertContracts()">🚀 Inserisci Contratti nel Database</button>
  ...
  <script>
    async function insertContracts() {
      ...
      btn.disabled = true
      btn.textContent = '⏳ Inserimento in corso...'
      resultDiv.innerHTML = ''
      ...
      try {
        ...
        btn.disabled = false
        btn.textContent = '🚀 Inserisci Contratti nel Database'
      }
    }
  </script>
</body>
...
// GET /test-fix-3-contracts - Pagina correzione 3 contratti sbagliati
app.get('/test-fix-3-contracts', async (c) => {
  return c.html(`<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>🔧 Correggi 3 Contratti Manuali</title>
  <style>
...
<body>
  <div class="container">
    <h1>🔧 Correzione 3 Contratti Manuali</h1>
    
    <div class="error-box">
      <strong>🚨 Problema Rilevato:</strong><br>
    ...
    <div class="contract-list">
      <h3>📋 Contratti da CORREGGERE:</h3>
      <div class="contract-item">
        <strong>1. Alberto Locatelli (per assistito Giovanni)</strong><br>
        ❌ ID vecchio: <code>contract-locatelli-1771122740756</code><br>
        ✅ ID corretto: <code>CONTRACT_CTR-LOCATELLI-2026_[timestamp]</code><br>
        Data firma: <strong>03/02/2026</strong> → Scadenza: <strong>02/02/2027</strong><br>
        PDF: <code>03.02.2026_signor Locatelli_BASE_SIDLY VITAL CARE.pdf</code>
      </div>
      <div class="contract-item">
        <strong>2. Francesco Pepe</strong><br>
        ❌ ID vecchio: <code>contract-pepe-1771122740794</code><br>
        ✅ ID corretto: <code>CONTRACT_CTR-PEPE-2026_[timestamp]</code><br>
        Data firma: <strong>27/01/2026</strong> → Scadenza: <strong>26/01/2027</strong><br>
        PDF: <code>27.01.2026_Pepe Francesco Contratto.pdf</code>
      </div>
      <div class="contract-item">
        <strong>3. Claudio Macchi</strong><br>
        ❌ ID vecchio: <code>contract-macchi-1771122740833</code><br>
        ✅ ID corretto: <code>CONTRACT_CTR-MACCHI-2026_[timestamp]</code><br>
        Data firma: <strong>01/02/2026</strong> → Scadenza: <strong>31/01/2027</strong><br>
        PDF: <code>Documento x Claudio Macchi.pdf</code>
      </div>
    </div>

    <div class="fix-info">
      <strong>✅ Cosa farà questo tool:</strong>
      <ol>
    ...
    <button id="fixBtn" onclick="fixContracts()">🔧 Correggi i 3 Contratti</button>
    <button onclick="window.location.href='/admin/leads-dashboard'">📊 Vai alla Dashboard</button>
  ...
  <script>
    async function fixContracts() {
      ...
      try {
        ...
        btn.disabled = false;
        btn.textContent = '🔧 Correggi i 3 Contratti';
      }
    }
  </script>
</body>
  ...
  try {
    ...
    for (const contractData of contracts) {
      try {
        ...
        // URL del PDF (se fornito)
        const pdfUrl = contractData.pdfUrl || ''
...
app.delete('/api/leads/:id', async (c) => {
  ...
  try {
    ...
    // Elimina record correlati PRIMA del lead per evitare problemi FK
    try {
      // Elimina configurations associate (se esistono)
      await c.env.DB.prepare('DELETE FROM configurations WHERE leadId = ?').bind(id).run()
      console.log('✅ Configurations associate eliminate')
    } catch (e) {
      console.log('ℹ️ Nessuna configuration o tabella non esiste:', e)
    }
    
    try {
      // Elimina assistiti associati (se esistono)
      await c.env.DB.prepare('DELETE FROM assistiti WHERE lead_id = ?').bind(id).run()
      console.log('✅ Assistiti associati eliminati')
    } catch (e) {
      console.log('ℹ️ Nessun assistito o tabella non esiste:', e)
    }
  ...
  try {
    ...
    try {
      ...
      if (configExists) {
        ...
        // Ricrea con TUTTE le colonne (vecchie + nuove aggiunte)
        await c.env.DB.prepare(`
    ...
    try {
      ...
      try {
        const result = await c.env.DB.prepare(`
          UPDATE leads 
          SET hs_object_source_detail_1 = 'Form eCura'
        ...
        `).run()
        console.log(`✅ Popolato hs_object_source_detail_1 = 'Form eCura' per ${result.meta?.changes || 0} lead esistenti (dal 29/01/2026)`)
